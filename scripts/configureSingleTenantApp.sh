...
az bot create --name $BOT_NAME --resource-group $AZURE_RESOURCE_GROUP_NAME --app-type SingleTenant --appid $CLIENT_ID --tenant-id $AZURE_TENANT_ID -e https://$BACKEND_APP_NAME.azurewebsites.net/api/messages -l global

# Configure the App Service with Single Tenant app credentials.
az webapp config appsettings set -g $AZURE_RESOURCE_GROUP_NAME -n $BACKEND_APP_NAME --settings MicrosoftAppId=$CLIENT_ID MicrosoftAppPassword=$CLIENT_SECRET MicrosoftAppType=SingleTenant